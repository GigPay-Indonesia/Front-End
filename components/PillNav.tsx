...
export interface PillNavProps {
    logo?: React.ReactNode; // Modified to accept ReactNode
    items: PillNavItem[];
    activeHref?: string;
    className?: string;
    ease?: string;
    ...
    rightContent?: React.ReactNode; // Added for Wallet Connect
}
...
const PillNav: React.FC<PillNavProps> = ({
    ...
    useEffect(() => {
        ...
        if (initialLoadAnimation) {
            ...
            if (rightEl) {
                gsap.set(rightEl, { opacity: 0, x: 20 });
                gsap.to(rightEl, {
                    opacity: 1,
                    x: 0,
                    duration: 0.6,
    ...
    const cssVars = {
        ...
        ['--nav-h']: '48px', // Increased slightly for better spacing
        ['--logo']: '36px',
        ['--pill-pad-x']: '20px',
        ['--pill-gap']: '4px'
    ...
    return (
        <div className="fixed top-4 z-[50] w-full left-0 px-4">
            <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 items-center">
                ...
                {/* Center: Nav Items (hidden on mobile, centered on desktop) */}
            ...
            <div
                ref={mobileMenuRef}
                className="md:hidden fixed top-[5em] left-4 right-4 rounded-[27px] shadow-[0_8px_32px_rgba(0,0,0,0.12)] z-[998] origin-top border border-white/10"
                style={{
                    ...cssVars,
                    background: 'var(--base, #000)'
                }}
            >
                <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
                                ...
                                {isRouterLink(item.href) ? (
                                    <Link
                                        ...
                                        onClick={() => {
                                            ...
                                            item.onClick && item.onClick({ preventDefault: () => { } } as React.MouseEvent);
                                        }}
                                    >
                                        {item.label}
                                    </Link>
                    ...
                    {rightContent && (
                        <li className="p-3 flex justify-center bg-white/5 rounded-[50px]">
                            {rightContent}
                        </li>
                    )}
                </ul>
